...
var priceHistory = function () {
    ...
    this.addTf = function (priceObj, tf, millsecs) {
        ...
        if (!priceCollections[symbol][tf]) {
            ...
            setTFTime(priceObj, millsecs);//new
    ...
    var setTFTime = function(priceObj, millsecs){
      if (millsecs) {
            //set the exact timeframe price time - ie choppingo off any  extra.
            //e.g for 1min timeframe 24:50:24 becomes 24:50:00, the exact time.
            // The extra 24 seconds will be chopped off.

            //now chop off the extra
            priceObj.tf_time = priceObj.time - (priceObj.time % millsecs);
        }  
    };

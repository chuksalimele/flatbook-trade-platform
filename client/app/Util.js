...
Ext.define('TradeApp.Util', {
    ...
    numBars: function () {
        ...
        return 100; //we are using highcharts
    },
    ...
    requestHighchartsTimeframeData: function (symbol, cbo, cbo_val, callback) {
        if (this.isRequestingTF) {
            return;
        }
        this.isRequestingTF = true;
    ...
    toHighchartsTFData: function(tf_data){
        var arr = [];
        for (var i = 0; i < tf_data.length; i++) {            
            arr.push([
                tf_data[i].tf_time,//uncomment later
                //tf_data[i].time - (tf_data[i].time % 60000),//testing
                tf_data[i].open,
                tf_data[i].high,
                tf_data[i].low,
                tf_data[i].close
    ...
    tfPriceData: function (symbol, tf) {
        ...
        if (this.tfData && this.tfData[symbol] && this.tfData[symbol][tf]) {
            ...
            try {
                var tf_data_str = window.localStorage.getItem(this.tfKey(symbol, tf));
                return JSON.parse(tf_data_str);
            } catch (e) {
                console.log(e);
            }

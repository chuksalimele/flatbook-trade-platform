...
module.exports = function (_sObj) {
    ...
    this.currentRebate = function (input, res) {
        ...
        var now = new Date(nowTimeUTC);
        var year = now.getFullYear();
        var month = now.getMonth() + 1; // being zero based month , we add 1
        var str_month = month < 10 ? "0" + month : "" + month;
        var monthBeginDate = year + "-" + str_month + "-" + "01" + " 00:00:00";

        var toDateTime = sObj.now();//now
        var rebate = {};

        totalLiveCommission('1970-01-01 00:00:00', toDateTime)
                .then(function (result) {
                    rebate.total_commission = result;
                    return totalLiveCommission(monthBeginDate, toDateTime);
                })
                .then(function (result) {
                    rebate.month_commission = result;
